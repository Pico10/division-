...
import { type } from "os";
import carritoDetallesmodels from "../Models/Carrito-Detalles";
//Para poder obtener los products de la base de datos, deberé importar el models de producto
import productsmodels from "../Models/products";
import carritomodels from "../Models/Carrito";
...
const carrito_DetailsController = {
        ...
        get: async (req: Request, res: Response) => {
            try
            {
                const existeCarrito = await carritomodels.find()
                if(existeCarrito)
                {
                //Obtener products
                const buscarproducts = await carritoDetallesmodels.find()
        ...
        add: async (req: Request, res: Response) => {
            try
            {
                ...
                const buscarproducts = await productsmodels.findOne({Nombre_Producto: req.body.Nombre_Producto});
                const existeCarrito = await carritomodels.findOne({Nombre_Carrito: "Carrito"})
                                    //Consultas mongoose
                if(!existeCarrito)
                {
                    crearCarrito();
                ...
                if(buscarproducts?.Nombre_Producto && existeCarrito){
                //OperacionesCART es la cantidad de products que se quiuere tener ahora y también tendrá el precio que se obtendrá de la base de datos products
                const OperacionesCART = {Cantidad: req.body.Cantidad, Precio: buscarproducts.Precio}
                if(OperacionesCART.Cantidad <= buscarproducts.Cantidad ){
                    //Si se cumple la condición, se agregará el producto y se descontará la cantidad de products del model producto                   
                    //La variable TotalPrecio guardará el nuevo precio según la cantidad de products ingresados
                    ...
                    const addProducto = new carritoDetallesmodels({Nombre_Producto: buscarproducts?.Nombre_Producto, Cantidad: req.body.Cantidad, Precio: TotalPrecio}); 
                    ...
                    const stockRestante = getstockRestante(OperacionesCART.Cantidad, buscarproducts.Cantidad);
                    ...
                    if(stockRestante == 0)
                    {
                        ...
                        existeCarrito.save();
                        buscarproducts.delete();
                        
                    }else{
                        buscarproducts.Cantidad = stockRestante;
                        buscarproducts.save();
                        existeCarrito?.Carrito_Details.push(addProducto)
                        existeCarrito.Precio_Total = getPreciocarritoList(existeCarrito.Precio_Total, TotalPrecio);
                        existeCarrito.save();
        ...
        delete: async (req: Request, res: Response) => {
            try
            {
                ...
                const BuscarProducto = await carritoDetallesmodels.findOne({Nombre_Producto: req.params.Nombre_Producto})
                //Antes de eliminarlo creo también una variable que conecta con la base de datos de Producto
                const products = await productsmodels.findOne({Nombre_Producto: req.params.Nombre_Producto})
                const existeCarrito = await carritomodels.findOne({Nombre_Carrito: "Carrito"})
                if(!existeCarrito)
                {
                    crearCarrito()
                ...
                if(BuscarProducto?.Nombre_Producto && products?.Nombre_Producto){
                //Creo la variable del producto del carrito que se está por eliminar para devolver el stock en la base de datos de Producto
                    ...
                    const Stockproducts = {Cantidad: products?.Cantidad}
                    ...
                    const TotalStock = getCantidadTOTAL(StockCarrito.Cantidad, Stockproducts.Cantidad);
                    const productoNombre = await carritoDetallesmodels.findOneAndDelete({Nombre_Producto: req.params.Nombre_Producto})
                    //Una vez eliminado la base de datos, se guardará los stock sumados a la base de datos de products
                    products.Cantidad = TotalStock;
                ...
                }else if(BuscarProducto?.Nombre_Producto && !products?.Nombre_Producto && BuscarProducto.Precio) 
                {
                    ...
                    const newProducto = new productsmodels({Nombre_Producto: BuscarProducto.Nombre_Producto, Cantidad: BuscarProducto.Cantidad, Precio: precioProducto});
                    newProducto.save();
                    //Actualizar array
                    if(existeCarrito)
                    {
        ...
        put: async (req: Request, res: Response) => {
            try
            {
                ...
                const obtenerProductoCART = await carritoDetallesmodels.findOne({Nombre_Producto: req.body.Nombre_Producto})
                //Se obtendrá el producto de la base de datos de producto
                const obtenerProducto = await productsmodels.findOne({Nombre_Producto: req.body.Nombre_Producto})
                ...
                if(obtenerProductoCART && obtenerProducto)
                {
                    ...
                    //En el siguiente condicional, se comprobará que la cantidad nueva de products no sea negativa
                ...
                if(obtenerProductoCART && !obtenerProducto)
                {
                    ...
                    const stockRestante = getstockRestante(nuevaCantidad.Cantidad, stockTOTAL)
                    const precioCARTAntiguo = obtenerProductoCART.Precio;
                    //En el siguiente condicional, se comprobará que la cantidad nueva de products no sea negativa ni 0
                    //También si el precio del producto del carrito existe y también el precio del producto de la base de datos
                    if(stockRestante > 0)
                    {
                        //Sí la nueva cantidad es 0, entonces se borrará el producto de la base de datos y se devolverá todo sus valores a la base de datos products
                        if(nuevaCantidad.Cantidad == 0 && existeCarrito && obtenerProductoCART.Precio)
                        {
                            const crearProducto = new productsmodels({Nombre_Producto: obtenerProductoCART.Nombre_Producto, Cantidad: stockRestante, Precio: precioProducto})
                            crearProducto.save();
                           //Eliminar valores del array
                            existeCarrito.Carrito_Details = existeCarrito.Carrito_Details.filter((producto) => {
                            return producto.Nombre_Producto != obtenerProductoCART.Nombre_Producto
...
{
    const newCarrito = new carritomodels({Nombre_Carrito: "Carrito", Carrito_Details: [], Precio_Total: 0}); 
    newCarrito.save();
}
